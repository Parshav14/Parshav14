<!-- ====================== HEADER ====================== -->

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:020617,100:111827&height=260&section=header&text=Parshav%20Singla&fontSize=52&fontAlignY=38&fontColor=ffffff&desc=AI%20Engineer%20%7C%20ML%20Researcher%20%7C%20Software%20Developer&descAlignY=60&descAlign=50" alt="Parshav Singla Header" />
</p>

<p align="center">
  <a href="https://parshavsingla.vercel.app" target="_blank">
    <img src="https://img.shields.io/badge/Portfolio-020617?style=for-the-badge&logo=vercel&logoColor=ffffff" />
  </a>
  <a href="https://www.linkedin.com/in/parshavsingla" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=ffffff" />
  </a>
  <a href="mailto:parshavsingla1@gmail.com">
    <img src="https://img.shields.io/badge/Email-111827?style=for-the-badge&logo=gmail&logoColor=ffffff" />
  </a>
  <a href="https://github.com/Parshav14" target="_blank">
    <img src="https://img.shields.io/badge/GitHub-020617?style=for-the-badge&logo=github&logoColor=ffffff" />
  </a>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Role-AI%20%2F%20ML%20Engineer-1f2937?style=flat-square" />
  <img src="https://img.shields.io/badge/Focus-LLMs%20%7C%20Speech%20%7C%20NLP-111827?style=flat-square" />
  <img src="https://img.shields.io/badge/Base-India-020617?style=flat-square" />
</p>

---

<!-- ====================== HERO SECTION ====================== -->

<div align="center">
  <img src="https://media.tenor.com/qJ5evVs-_uUAAAAC/coding.gif" width="420" alt="Coding gif" />
</div>

<br/>

<div align="center">

**AI Engineer · ML Researcher · Software Builder**

I like building things that think, listen, speak, and scale.

</div>

---

<!-- ====================== TL;DR ====================== -->

## 🧾 TL;DR – Who Am I?

```yaml
name: "Parshav Singla"
role: "AI / ML Engineer, Research-Oriented Software Developer"
education:
  degree: "B.E. in Computer Science and Engineering"
  institute: "Thapar Institute of Engineering and Technology, Patiala"
  graduation: "2026 (expected)"
experience:
  - "Samsung PRISM – ML Research & Speech/NLP"
  - "ISRO – Network Security & Vulnerability Assessment"
interests:
  - "LLMs & NLP"
  - "Speech/TTS & accessibility"
  - "ML systems & evaluation"
  - "Scalable backend + data platforms"
values:
  - "Data > opinion"
  - "Clarity over complexity"
  - "Ship, measure, iterate"
````

---

<!-- ====================== ABOUT ====================== -->

## 👨‍💻 About Me

* 🎓 Final-year **Computer Science Engineer** at **Thapar Institute of Engineering and Technology**
* 🤖 Working across **Machine Learning, Deep Learning, NLP, Speech Processing, LLMs & Data Systems**
* 🧪 **Published author (Springer LNNS, ICICC 2025)** through the **Samsung PRISM** program
* 🛰️ **Research Intern @ ISRO**, building **IP-scanning & vulnerability assessment tools** for secure networks
* 🧱 Enjoy taking ideas from **research → prototype → API → production-grade product**
* 🎯 Interested in **AI for accessibility**, **intelligent automation**, **data quality**, and **robust ML systems**
* 📫 Reach me at: **`parshavsingla1@gmail.com`**

---

<!-- ====================== NOW ====================== -->

## 🔭 What I’m Currently Focused On

* Building and refining **Vision2VoiceAI** – image → audio narration with **BLIP + TTS + multilingual** support
* Exploring **LLMs**, prompt engineering, and retrieval for **practical, user-centric AI workflows**
* Improving my understanding of **ML evaluation, drift, and data validation at scale**
* Sharpening **backend + systems design** for scalable AI-based applications

---

<!-- ====================== EXPERIENCE ====================== -->

## 💼 Experience

### 🧠 Samsung PRISM – Research Intern

**Samsung Research and Development Institute, India (Remote) | Aug 2023 – May 2025**

* Built **ML components** for **speech, text, and video summarization** pipelines
* Achieved a **PESQ score of 4.64** using Hi-Fi GAN for speech enhancement in conversational TTS
* Co-authored a **Springer LNNS (ICICC 2025)** paper on **converting read text to conversational dialogue**
* Improved internal video summarization flows, **reducing review time by ~50%**
* Worked closely with product-aligned teams, focusing on **reliability, evaluation and deployable research**

---

### 🛰️ ISRO – Research Intern

**Indian Space Research Organization, Sriharikota | Dec 2024 – Jan 2025**

* Designed and implemented an **Advanced IP Scanning system** for live network environments
* Built a **web-based IP tracking solution** for **real-time vulnerability assessment**
* Improved detection accuracy and responsiveness, supporting **stronger cybersecurity posture**

---

### 🧭 Leadership – Head of Marketing & Public Relations

**IETE Student Chapter, Thapar | Jul 2023 – May 2025**

* Led communication, marketing & events, bringing **450+ students** into technical activities
* Managed sponsorship outreach with brands like **Wildcraft** and **Red Bull**
* Mentored juniors, coordinated technical workshops, and helped build a **collaborative learning culture**

---

<!-- ====================== PROJECTS ====================== -->

## 🧩 Selected Projects

### 🔊 Vision2VoiceAI – Image to Audio Narration

> `Multimodal · BLIP · Neural TTS · Accessibility`

* Converts **images → speech** using caption generation, language translation & neural TTS
* Supports **20+ languages** with ~**3s average audio generation** time
* Designed for **accessibility and inclusive content consumption**
* Worked with **audio preprocessing, noise reduction & clean TTS training data**
  🔗 **Live:** [https://vision2voiceai.vercel.app](https://vision2voiceai.vercel.app)

---

### 🚚 AirCario – Full-Stack Logistics & Courier Platform

> `Node.js · PostgreSQL · WebSockets · Scalable Backend`

* Built an end-to-end **courier & logistics management system**
* Real-time shipment tracking with **role-based dashboards** (admin/driver/user)
* Backend designed for **10k+ RPS, 50M+ potential users & 100k drivers**
* Implemented **sharding, caching, load balancing & WebSocket GPS updates**
  🔗 **Live:** [https://aircario.vercel.app](https://aircario.vercel.app)

---

### 🗣️ Text-to-Speech Accessibility System

> `Python · TTS · Accessibility · Copyrighted`

* Developed a **Text-to-Speech solution** to enhance content accessibility
* Focused on **natural voice output**, consistent quality & user-friendly interface
* Awarded **software copyright** by the **Government of India**

---

<!-- ====================== SKILLS ====================== -->

## 🛠️ Tech Stack – Dark Mode

### 💻 Languages

<p align="center">
  <img src="https://img.shields.io/badge/Python-000000?style=for-the-badge&logo=python&logoColor=ffdd54" />
  <img src="https://img.shields.io/badge/C++-020617?style=for-the-badge&logo=cplusplus&logoColor=00599C" />
  <img src="https://img.shields.io/badge/JavaScript-111827?style=for-the-badge&logo=javascript&logoColor=F7DF1E" />
  <img src="https://img.shields.io/badge/SQL-020617?style=for-the-badge&logo=postgresql&logoColor=4169E1" />
</p>

### 🤖 AI / ML / Data

<p align="center">
  <img src="https://img.shields.io/badge/PyTorch-0f172a?style=for-the-badge&logo=pytorch&logoColor=EE4C2C" />
  <img src="https://img.shields.io/badge/TensorFlow-020617?style=for-the-badge&logo=tensorflow&logoColor=FF6F00" />
  <img src="https://img.shields.io/badge/Scikit--Learn-111827?style=for-the-badge&logo=scikitlearn&logoColor=F7931E" />
  <img src="https://img.shields.io/badge/HuggingFace-020617?style=for-the-badge&logo=huggingface&logoColor=FFCC4D" />
  <img src="https://img.shields.io/badge/NumPy-020617?style=for-the-badge&logo=numpy&logoColor=4fa1ff" />
  <img src="https://img.shields.io/badge/Pandas-020617?style=for-the-badge&logo=pandas&logoColor=white" />
  <img src="https://img.shields.io/badge/OpenCV-020617?style=for-the-badge&logo=opencv&logoColor=5C3EE8" />
</p>

### 🌐 Backend · APIs · Web

<p align="center">
  <img src="https://img.shields.io/badge/Node.js-020617?style=for-the-badge&logo=nodedotjs&logoColor=339933" />
  <img src="https://img.shields.io/badge/Express-020617?style=for-the-badge&logo=express&logoColor=ffffff" />
  <img src="https://img.shields.io/badge/PostgreSQL-020617?style=for-the-badge&logo=postgresql&logoColor=4169E1" />
  <img src="https://img.shields.io/badge/MongoDB-020617?style=for-the-badge&logo=mongodb&logoColor=47A248" />
  <img src="https://img.shields.io/badge/Streamlit-020617?style=for-the-badge&logo=streamlit&logoColor=FF4B4B" />
  <img src="https://img.shields.io/badge/Flask-020617?style=for-the-badge&logo=flask&logoColor=ffffff" />
</p>

### ☁️ DevOps · Cloud · Tools

<p align="center">
  <img src="https://img.shields.io/badge/AWS-020617?style=for-the-badge&logo=amazonaws&logoColor=FF9900" />
  <img src="https://img.shields.io/badge/Azure-020617?style=for-the-badge&logo=microsoftazure&logoColor=0078D4" />
  <img src="https://img.shields.io/badge/Docker-020617?style=for-the-badge&logo=docker&logoColor=2496ED" />
  <img src="https://img.shields.io/badge/Linux-020617?style=for-the-badge&logo=linux&logoColor=FCC624" />
  <img src="https://img.shields.io/badge/Git-020617?style=for-the-badge&logo=git&logoColor=F05032" />
  <img src="https://img.shields.io/badge/GitHub-020617?style=for-the-badge&logo=github&logoColor=ffffff" />
</p>

---

<!-- ====================== ACHIEVEMENTS ====================== -->

## 🏅 Highlights & Achievements

* 📄 **Springer LNNS Publication (ICICC 2025)** – “Bridging the Gap: Converting Read Text to Conversational Dialogue” under **Samsung PRISM**
* 🏆 **EY Techathon 5.0 Semi-finalist** – ranked among the **top teams from 6,500+ teams and 1.5 lakh+ registrations**
* 🥇 **Samsung PRISM Excellence Category** – recognized for high-impact ML work on speech conversion
* 🧾 **Government of India Software Copyright** – Text-to-Speech accessibility system
* 📜 Certifications: **NVIDIA – Getting Started with Deep Learning**, **Cisco – Cybersecurity Essentials**, **Microsoft – Azure AI Fundamentals**

---

<!-- ====================== TIME-LINE ====================== -->

## 📆 Journey So Far (Timeline)

```text
2021–2022  | Class XII – 93.4% (Science), foundation in PCM
2022       | Joined B.E. CSE @ Thapar Institute of Engineering and Technology
2023       | Started Samsung PRISM – ML research in speech & NLP
2023–2025  | Head of Marketing & PR @ IETE Student Chapter
2024       | EY Techathon 5.0 – Semi-finalist
Dec 2024   | Research Intern @ ISRO – Network security tools
2025       | Springer LNNS Publication (ICICC) under Samsung PRISM
Now        | Building ML-driven products & preparing for AI / ML / SWE roles
```

---

<!-- ====================== GITHUB ANALYTICS ====================== -->

## 📊 GitHub Analytics – Dark Mode

<div align="center">

<!-- Core stats -->

<img src="https://github-readme-stats.vercel.app/api?username=Parshav14&show_icons=true&theme=github_dark&hide_border=true&include_all_commits=true&count_private=true" alt="GitHub Stats" />

<br/><br/>

<!-- Streak -->

<img src="https://github-readme-streak-stats.herokuapp.com/?user=Parshav14&theme=dark&hide_border=true" alt="GitHub Streak" />

<br/><br/>

<!-- Top languages -->

<img src="https://github-readme-stats.vercel.app/api/top-langs?username=Parshav14&show_icons=true&locale=en&layout=compact&theme=github_dark&hide_border=true" alt="Top Languages" />

<br/><br/>

<!-- Activity graph -->

<img src="https://github-readme-activity-graph.vercel.app/graph?username=Parshav14&bg_color=020617&color=38bdf8&line=22c55e&point=ffffff&area=true&hide_border=true" alt="Contribution Graph" />

<br/><br/>

<!-- Profile summary -->

<img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=Parshav14&theme=github_dark" alt="Profile Summary" />

</div>

---

<!-- ====================== TROPHIES ====================== -->

## 🏆 GitHub Trophies

<p align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=Parshav14&theme=onestar&no-frame=true&no-bg=true&row=1&column=7" alt="GitHub Trophies" />
</p>

---

<!-- ====================== WORK ETHIC / MINDSET ====================== -->

## 🧭 How I Work

```yaml
mindset:
  - "Be honest with data and results."
  - "Simple > clever, reliable > experimental (in production)."
  - "Prefer momentum with iteration over stuck perfection."

strengths:
  - "Debugging weird ML behavior and data issues"
  - "Connecting research insights to product requirements"
  - "Writing clear documentation and reproducible experiments"
  - "Collaborating across engineering, research, and product"

looking_for:
  - "AI / ML Engineer roles"
  - "Data Science roles with real ownership"
  - "Research Engineer positions around LLMs / speech / NLP"
  - "SWE roles where ML and data matter"
```

---

<!-- ====================== CONTACT ====================== -->

## 🤝 Let’s Connect

<p align="left">
  <a href="https://www.linkedin.com/in/parshavsingla" target="_blank">
    <img src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/linked-in-alt.svg" alt="LinkedIn" height="32" width="42" />
  </a>
  <a href="https://kaggle.com/parshavsingla" target="_blank">
    <img src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/kaggle.svg" alt="Kaggle" height="32" width="42" />
  </a>
  <a href="https://twitter.com/parshav769" target="_blank">
    <img src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/twitter.svg" alt="Twitter" height="32" width="42" />
  </a>
  <a href="https://instagram.com/parshav_789" target="_blank">
    <img src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/instagram.svg" alt="Instagram" height="32" width="42" />
  </a>
  <a href="https://leetcode.com/u/Parshav_singla01/" target="_blank">
    <img src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/leet-code.svg" alt="LeetCode" height="32" width="42" />
  </a>
</p>

---

<p align="center">
  <i>Thanks for visiting. If something here sparked an idea or aligns with what you’re building, feel free to reach out.</i>
</p>

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:111827,100:020617&height=140&section=footer" />
</p>
